...
{
    public class DuplicateSizeByUser : IStatChart
    {
        public string Title
        {
            get { return "Volume de doublons par utilisateur"; }
        }
        ...
        public string Sql
        {
            ...
            {
                return
                    "select sum(taille) as Volume,title as Utilisateur from duplicatefile.analyse join user on user.login=analyse.user where isduplicate=1 and isoriginal<>1 group by user order by volume desc limit 30";
            }
        }
